...
internal class BallGO : GameObject
{
	public BallGO(Scene scene, Transform transform) : base(scene, transform)
	{
		...
		rb.CreateCircule(Math.Max(sr.m_Size.X, sr.m_Size.Y) / 2, 0.0f, 0.0f, Vector2.Zero, Physics.GetCategoryByName("WorldObject"), Physics.GetCategoryByName("WorldObject") | Physics.GetCategoryByName("Player") | Physics.GetCategoryByName("Wall"));
